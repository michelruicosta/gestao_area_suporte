# -*- coding: utf-8 -*-
"""
Triagem automática — categoria **DDR** (CADOC ``DDR_2011``).
...
REGRAS_CONCLUIR: TabelaConcluir = {
    ...
    "ultima_cliente_para_finaud": [
        ...
        Regra(2, "§4e DDR: agradecimento sem novo pedido no texto",
              _det_4e_ddr,
              motivo="DDR §4e: agradecimento sem novo pedido no texto"),
        Regra(3, "G3: cliente confirma concordância após instrução Finaud",
              _det_g3_par_conclusivo,
              motivo="DDR G3: cliente disse 'de acordo' após instrução da Finaud — penúltima é Finaud"),
    ],
}
...
REGRAS_AGUARDANDO: TabelaAguardando = {
    "ultima_finaud_para_cliente": [
        ...
        Regra(3, "DDR §3.5+: Finaud só agradece (sem C→F prévio)",
              _det_35_plus_agradece_sem_cliente_previa, fila="finaud",
              motivo="DDR §3.5 — agradecimento sem remessa, sem C→F prévio"),
    ],
...
_FRASES_AGUARDANDO: Dict[Tuple[Bucket, int], str] = {
    ...
    ("ultima_finaud_para_cliente", 3):
        "Triagem automática (DDR): Finaud só agradece — aguarda envio de arquivo ao cliente.",
    ("ultima_finaud_interna", 1):
        "Triagem automática: última mensagem interna Finaud→Finaud — aguarda tratamento (DDR).",
