# -*- coding: utf-8 -*-
"""
Triagem automática — categoria **4111** (CADOC ``4111``).

Características próprias:
  - **§4e DDR** — agradecimento sem novo pedido fecha;
  - sem regra F→C substantiva;
  - F→F tratado com lógica nuançada;
  - default ``sec35_agradecimento_sem_msg_cliente_previa=False`` (§3.5+ off);
  - inclui §6 (cluster empresa+lista_prazos) e §6b (cluster núcleo-assunto).
"""
...
SEC35_SEM_MSG_CLIENTE_PREVIA = False
...
def _det_4e_ddr(ctx: Contexto) -> bool:
    """§4e DDR: cliente só agradeceu após remessa."""
    return _cliente_somente_reconhecimento_curto_pos_remessa(ctx.ultima_msg or {})
...
REGRAS_CONCLUIR: TabelaConcluir = {
    ...
    "ultima_cliente_para_finaud": [
        ...
        Regra(2, "§4e DDR: agradecimento sem novo pedido no texto",
              _det_4e_ddr,
              motivo="4111 §4e: agradecimento sem novo pedido no texto"),
        Regra(3, "G3: cliente confirma concordância após instrução Finaud",
              _det_g3_par_conclusivo,
              motivo="4111 G3: cliente disse 'de acordo' após instrução da Finaud — penúltima é Finaud"),
    ],
}
...
REGRAS_AGUARDANDO: TabelaAguardando = {
    "ultima_finaud_para_cliente": [
        ...
        Regra(3, "4111 §3.5+: Finaud só agradece (sem C→F prévio)",
              _det_35_plus_agradece_sem_cliente_previa, fila="finaud",
              motivo="4111 §3.5 — agradecimento sem remessa, sem C→F prévio"),
    ],
...
_FRASES_AGUARDANDO: Dict[Tuple[Bucket, int], str] = {
    ...
    ("ultima_finaud_para_cliente", 3):
        "Triagem automática (4111): Finaud só agradece — aguarda envio de arquivo ao cliente.",
    ("ultima_finaud_interna", 1):
        "Triagem automática: última mensagem interna Finaud→Finaud — aguarda tratamento (4111).",
