...
def test_passo_b_solicita_extrato():
    """Passo B: Finaud usa 'vou precisar' sem anexo → AC submotivo solicitou planilha."""
    msgs = [
    ...
    status, motivo = bt._determinar_status(msgs)
    assert status == 'Aguardando Cliente'
    assert motivo == 'Finaud solicitou extrato ou planilha — aguarda envio'


def test_passo_b_orientacao_tecnica():
    """Passo B: Finaud usa 'orientamos que' → AC submotivo orientação técnica."""
    msgs = [
    ...
    status, motivo = bt._determinar_status(msgs)
    assert status == 'Aguardando Cliente'
    assert motivo == 'Finaud deu orientação técnica — aguarda execução'


def test_passo_b_solicita_planilha():
    """Passo B: Finaud usa 'solicitamos encaminhar' → AC submotivo solicitou planilha."""
    msgs = [
        _msg('cliente@empresa.com', corpo='Bom dia, estou com dúvida.', assunto='DLO'),
        _msg(FINAUD, corpo='Prezado, solicitamos encaminhar a planilha LEC do mês de maio para análise.', assunto='Re: DLO'),
    ]
    status, motivo = bt._determinar_status(msgs)
    assert status == 'Aguardando Cliente'
    assert motivo == 'Finaud solicitou extrato ou planilha — aguarda envio'
...
def test_passo_b_proposta_reuniao():
    """Passo B: Finaud menciona 'reunião' sem anexo → AC submotivo proposta de reunião."""
    msgs = [
    ...
    status, motivo = bt._determinar_status(msgs)
    assert status == 'Aguardando Cliente'
    assert motivo == 'Finaud propôs reunião ou ligação — aguarda confirmação'
